...
describe 'Usuáro cadastra taxa fixa' do
  it 'a partir da tela inicial' do
    ...
    SedexDez.delete_all
    sedex_dez = SedexDez.create!

    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Cadastrar taxa fixa', href: edit_sedex_dez_path(sedex_dez.id))
  ...
  it 'mas já foi feito o cadastro' do
    ...
    SedexDez.delete_all
    sedex_dez = SedexDez.create!(flat_fee:40)
    ...
    expect(page).to have_link('Editar', href: edit_sedex_dez_path(sedex_dez.id))
    expect(page).not_to have_link('Cadastrar taxa fixa')
  end
  it 'e mantém valores válidos' do
    ...
    expect(current_path).to eq sedex_dez_path(sedex_dez.id)
  end
end