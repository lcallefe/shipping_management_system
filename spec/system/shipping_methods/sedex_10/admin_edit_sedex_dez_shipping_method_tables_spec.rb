...
describe 'Usuáro altera configuração para modalidade de transporte SedexDez' do
  it 'de distância x preço' do
    ...
    s = SedexDez.create!(flat_fee:45)
    pd = SedexDezPriceDistance.create!(min_distance:10, max_distance:30, price:40, sedex_dez_id:s.id)
                    
    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Editar intervalo', href: edit_sedex_dez_price_distance_path(pd.id))
  ...
  it 'de distância x prazo' do
    ...
    s = SedexDez.create!(flat_fee:45)
    dt = SedexDezDeliveryTimeDistance.create!(min_distance: 10, max_distance: 20, delivery_time: 15, sedex_dez_id:s.id)
                
    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Editar intervalo', href: edit_sedex_dez_delivery_time_distance_path(dt.id))
  ...
  it 'de peso x preço' do
    # Arrange 
    user = User.create!(email:'carlasouza@sistemadefrete.com.br', name:'Carla', password:'abobrinha123', admin:true)
    s = SedexDez.create!(flat_fee:80)
    pw = SedexDezPriceWeight.create!(min_weight:5, max_weight:15, price:25, sedex_dez_id:s.id)
                
    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Editar intervalo', href: edit_sedex_dez_price_weight_path(pw.id))
  ...
  it 'e atualiza com dados inválidos para distância x prazo' do
    ...
    s = SedexDez.create!(flat_fee:45)
    dt = SedexDezDeliveryTimeDistance.create!(min_distance: 10, max_distance: 20, delivery_time: 15, sedex_dez_id:s.id)
    SedexDezDeliveryTimeDistance.create!(min_distance: 21, max_distance: 30, delivery_time: 25, sedex_dez_id:s.id)
    ...
    expect(page).to have_content('Distância mínima')
    expect(page).to have_content('Prazo')
    expect(page).not_to have_link('Voltar', href: sedex_dezs_path)
  ...
  it 'e atualiza com dados inválidos para peso x valor por km' do
    ...
    expect(current_path).not_to eq sedex_dezs_path
  end
