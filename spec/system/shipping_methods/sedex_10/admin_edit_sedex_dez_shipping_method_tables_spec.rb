...
describe 'Usuáro altera configuração para modalidade de transporte SedexDez' do
  it 'de distância x preço' do
    ...
    pd = FirstPriceDistance.create!(min_distance:10, max_distance:30, price:40, sedex_dez_id:s.id)
                    
    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Editar intervalo', href: edit_first_price_distance_path(pd.id))
  ...
  it 'de distância x prazo' do
    ...
    s = SedexDez.create!(flat_fee:45)
    dt = FirstDeliveryTimeDistance.create!(min_distance: 10, max_distance: 20, delivery_time: 15, sedex_dez_id:s.id)
                
    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Editar intervalo', href: edit_first_delivery_time_distance_path(dt.id))
  ...
  it 'de peso x preço' do
    ...
    pw = FirstPriceWeight.create!(min_weight:5, max_weight:15, price:25, sedex_dez_id:s.id)
                
    # Act
    login_as(user)
    visit root_path 
    click_on 'Sedex 10' 
    click_link('Editar intervalo', href: edit_first_price_weight_path(pw.id))
  ...
  it 'e atualiza com dados inválidos para distância x prazo' do
    ...
    s = SedexDez.create!(flat_fee:45)
    dt = FirstDeliveryTimeDistance.create!(min_distance: 10, max_distance: 20, delivery_time: 15, sedex_dez_id:s.id)
    FirstDeliveryTimeDistance.create!(min_distance: 21, max_distance: 30, delivery_time: 25, sedex_dez_id:s.id)
