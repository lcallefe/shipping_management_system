class SedexPriceDistance < ApplicationRecord
  belongs_to :sedex 
  ...
  def update_min_distance
    if SedexPriceDistance.count > 1
      last_max_distance = SedexPriceDistance.where("created_at < ?", self.created_at).order("id DESC").first.max_distance
      self.min_distance = last_max_distance + 1
    end
  end  
  def validate_price_distance_values
    model = SedexPriceDistance.all
    if model.count > 1
